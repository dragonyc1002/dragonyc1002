![IMG](./assets/sunset.JPG)  

## Welcome to my bio  
Hi there, welcome to my casual bio. I used to be more active on programming but the activity has incredibly decreased due to other commitments unfortunately. Hope to return coding someday though given how fun it has always been.

- [HBYC](https://github.com/HBYC-Team) is the most important project for me all the time. However, this project is currently pasued and do not have a confirmed timeframe to be continued in the near future until further notice.

- I'm currently learning JavaScript, alongside with some other programming languages.

## Github Stats  
![dragonyc1002's GitHub stats](https://github-readme-stats.vercel.app/api?username=dragonyc1002&show_icons=true&theme=dark)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=dragonyc1002&theme=dark)
